...
src.CSSTester.prototype = {
	updateHtml: function(html) {
		...
		var swfObject = new js.SWFObject("cocktail_browser.swf?html=" + urlEncodedHtml,"flash-test",400,400,"11","#FFFFFF");
		swfObject.addParam("wmode","transparent");
		swfObject.write("flash-test");
	...
	,onTestClick: function(e) {
		...
		e.preventDefault();
		var anchor = e.target;
		var http = new haxe.Http("proxy.php?url=" + anchor.href);
	...
	,initTestsBrowser: function(testsList) {
		...
		while( $it0.hasNext() ) {
			...
			if(chapter.get("name") != null) htmlTests += "<h3>" + chapter.get("name") + "</h3>";
			var $it1 = chapter.elements();
			while( $it1.hasNext() ) {
				var section = $it1.next();
				if(section.get("name") != null) htmlTests += "<h4>" + section.get("name") + "</h4>";
