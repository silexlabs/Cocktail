...
{
HX_STACK_PUSH("TouchInfo::new","native/display/Stage.hx",1062);
{
	HX_STACK_LINE(1062)
	this->touchOverObjects = Array_obj< ::native::display::InteractiveObject >::__new();
}
;
	return null();
}
...
Dynamic TouchInfo_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	switch(inName.length) {
	case 16:
		if (HX_FIELD_EQ(inName,"touchOverObjects") ) { touchOverObjects=inValue.Cast< Array< ::native::display::InteractiveObject > >(); return inValue; }
	}
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("native.display.TouchInfo"), hx::TCanCast< TouchInfo_obj> ,sStaticFields,sMemberFields,
