...
Void SWFPlugin_obj::onLoaderReady( ::native::display::Loader loader){
{
...
Void SWFPlugin_obj::onSWFLoadComplete( ::native::events::Event event){
{
...
Void SWFPlugin_obj::updateViewport( Float x,Float y,Float width,Float height){
{
		...
		if (  ( _switch_1==HX_CSTRING("noscale"))){
			HX_STACK_LINE(407)
			this->_swf->set_x(::Math_obj::round(this->viewport->x));
			HX_STACK_LINE(408)
			this->_swf->set_y(::Math_obj::round(this->viewport->y));
		}
			...
			this->_swf->set_y(::Math_obj::round(this->viewport->y));
			HX_STACK_LINE(413)
			this->_swf->set_scaleX((Float(this->viewport->width) / Float(this->_swfWidth)));
			HX_STACK_LINE(414)
			this->_swf->set_scaleY((Float(this->viewport->height) / Float(this->_swfHeight)));
		}
		else  {
			HX_STACK_LINE(417)
			this->_swf->set_x(::Math_obj::round((this->viewport->x + this->_swfBounds->x)));
			HX_STACK_LINE(418)
			this->_swf->set_y(::Math_obj::round((this->viewport->y + this->_swfBounds->y)));
			HX_STACK_LINE(419)
			this->_swf->set_scaleX((Float(this->_swfBounds->width) / Float(this->_swfWidth)));
			HX_STACK_LINE(420)
			this->_swf->set_scaleY((Float(this->_swfBounds->height) / Float(this->_swfHeight)));
		}
;
;
		HX_STACK_LINE(424)
...
Void SWFPlugin_obj::dispose( ){
{
		...
		HX_STACK_LINE(364)
		if (((::Std_obj::is(loadedSWF->response,hx::ClassOf< ::native::display::Loader >()) == true))){
			HX_STACK_LINE(365)
...
Void SWFPlugin_obj::detach( ::cocktail::core::graphics::GraphicsContext graphicsContext){
{
		...
		if (((this->_swfAddedToStage == true))){
			...
			if (((this->_isAVM1swf == false))){
				HX_STACK_LINE(335)
				this->_swf->get_parent()->removeChild(this->_swf);
			}
			else{
				HX_STACK_LINE(340)
				this->_loader->get_parent()->removeChild(this->_loader);
			}
			HX_STACK_LINE(343)
			this->_swfAddedToStage = false;
		}
...
Void SWFPlugin_obj::attach( ::cocktail::core::graphics::GraphicsContext graphicsContext){
{
		...
		if (((this->_swfAddedToStage == false))){
			HX_STACK_LINE(287)
			::native::display::DisplayObjectContainer containerGraphicContext = graphicsContext->get_nativeLayer();		HX_STACK_VAR(containerGraphicContext,"containerGraphicContext");
			HX_STACK_LINE(293)
			if (((bool((::cocktail::core::config::Config_obj::getInstance()->enableCompositing == false)) && bool((::cocktail::core::config::Config_obj::getInstance()->objectBelowWhenNoCompositing == true))))){
				...
				if (((this->_isAVM1swf == false))){
					HX_STACK_LINE(298)
					containerGraphicContext->get_parent()->addChildAt(this->_swf,(int)0);
				}
				else{
					HX_STACK_LINE(303)
					containerGraphicContext->get_parent()->addChildAt(this->_loader,(int)0);
				}
			}
			else{
				HX_STACK_LINE(308)
...
Void SWFPlugin_obj::init( ){
{
		...
		::cocktail::port::platform::nme::NativeHttp loadedSWF = ::cocktail::core::resource::ResourceManager_obj::getBinaryResource(this->_elementAttributes->get(HX_CSTRING("data")));		HX_STACK_VAR(loadedSWF,"loadedSWF");
		HX_STACK_LINE(174)
		if ((::Std_obj::is(loadedSWF->response,hx::ClassOf< ::native::display::Loader >()))){
			HX_STACK_LINE(176)
		...
		HX_STACK_LINE(189)
		this->_loader->contentLoaderInfo->addEventListener(::native::events::Event_obj::INIT,this->onSWFLoadComplete_dyn(),null(),null(),null());
		HX_STACK_LINE(193)
...
Dynamic SWFPlugin_obj::__SetField(const ::String &inName,const Dynamic &inValue,bool inCallProp)
{
	switch(inName.length) {
	case 4:
		if (HX_FIELD_EQ(inName,"_swf") ) { _swf=inValue.Cast< ::native::display::DisplayObject >(); return inValue; }
		break;
	case 5:
