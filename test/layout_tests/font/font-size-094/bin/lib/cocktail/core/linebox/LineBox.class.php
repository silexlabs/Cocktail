...
class cocktail_core_linebox_LineBox extends cocktail_core_dom_NodeBase {
	public function __construct($elementRenderer) {
		...
		parent::__construct();
		$this->bounds = _hx_anonymous(array("x" => 0.0, "y" => 0.0, "width" => 0.0, "height" => 0.0));
	...
	public function getBaselineOffset($parentBaselineOffset, $parentXHeight) {
		$baselineOffset = $parentBaselineOffset + $this->elementRenderer->coreStyle->computedStyle->verticalAlign;
		$»t = ($this->elementRenderer->coreStyle->verticalAlign);
		switch($»t->index) {
		case 5:
		{
