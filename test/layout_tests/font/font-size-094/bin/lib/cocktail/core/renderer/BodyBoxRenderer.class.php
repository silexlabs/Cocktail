...
class cocktail_core_renderer_BodyBoxRenderer extends cocktail_core_renderer_BlockBoxRenderer {
	...
	public function getBackgroundBounds() {
		...
		$bodyBounds = _hx_anonymous(array("x" => 0.0, "y" => 0.0, "width" => $width, "height" => $height));
		return $bodyBounds;
	}
	public function layoutSelf() {
		parent::layoutSelf();
		if($this->coreStyle->height == cocktail_core_style_Dimension::$cssAuto && ($this->isPositioned() === false || $this->isRelativePositioned() === true)) {
			$computedStyle = $this->coreStyle->computedStyle;
			$computedStyle->set_height($this->_containingBlock->getContainerBlockData()->height - $computedStyle->getMarginTop() - $computedStyle->getMarginBottom() - $computedStyle->getPaddingTop() - $computedStyle->getPaddingBottom());
		}
	}
