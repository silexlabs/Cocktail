...
*/
package cocktail.port.starling;
...
class GraphicsContextImpl extends AbstractGraphicsContextImpl
{
	...
	private static var _starling:starling.core.Starling;
	...
	public function new(isInitialGraphicContext:Bool) 
	{
		...
		if (_starling == null)
		{
			_starling = new starling.core.Starling(starling.display.Sprite, flash.Lib.current.stage);
			_starling.start();
		}
		...
		//var img = starling.display.Image.fromBitmap(_nativeBitmap);
		//_starling.stage.addChild(img);
		
	}
	
	private static var img:starling.display.Image;
	...
	private function updateB()
	{
		...
		haxe.Timer.delay(function() { 
			if (img == null)
			{
				texture = new starling.textures.RenderTexture(_width, _height, true);
				img = new starling.display.Image(texture);
			
			_starling.stage.addChild(img);
			}
			
			texture.draw(_nativeBitmap);
