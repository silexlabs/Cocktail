...
class Blob
{
   ...
   var mRect:RectangleData;
   var mPoint:PointData;
   ...
   public function draw(inX:Float,inY:Float)
   {
	  ...
	  mArena.drawImage(mBits.nativeElement);
   }
}
...
class Ski
{
	...
	// text implementation
	var mScoreText:TextElement;
	var mTopScoreText:TextElement;


	// Loads the input bitmap
	function new()
	{
		tileImage = new ImageDOMElement();
		
		tileImage.onLoad = this.init;
		tileImage.onError = initError;
		
		tileImage.load("tiles.png");
	}
	...
	*/
	function init(inBitmap:ImageDOMElement):Void
	{
		...
		var mScoreTextContainer:ContainerDOMElement = new ContainerDOMElement();
		mScoreText = new TextElement("");
		mScoreTextContainer.x = 10;
		mScoreTextContainer.y = 10;
		mScoreTextContainer.width = 300;

		rootDOMElement.addChild(mScoreTextContainer);

		var mTopScoreTextContainer:ContainerDOMElement = new ContainerDOMElement();
		mTopScoreText = new TextElement("");
		mTopScoreTextContainer.addText(mTopScoreText);
		mTopScoreTextContainer.x = 300;
		mTopScoreTextContainer.y = 10;
		mTopScoreTextContainer.width = 300;

		rootDOMElement.addChild(mTopScoreTextContainer);
	...
	function Render(inExtra:Float)
	{
	  ...
	  var fill:FillStyleValue = monochrome( cast { color:0xffffff, alpha:100 } );
	...
	function OnKeyDown(key:KeyboardEventData)
	{
	...
	// Key-up event
	function OnKeyUp(key:KeyboardEventData)
	{
	  // Store for use in game
