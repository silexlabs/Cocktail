...
cocktailCore.style.abstract.AbstractStyle.prototype = {
	...
	,initDefaultStyleValues: function() {
		...
		this.setBackgroundSize([cocktail.style.BackgroundSizeStyleValue.dimensions({ x : cocktail.style.BackgroundSizeStyleDimensionValue.auto, y : cocktail.style.BackgroundSizeStyleDimensionValue.auto})]);
	...
	,insertDOMElement: function(formattingContext,lastPositionedDOMElementData,viewportData) {
		if(this.isFloat() == true) formattingContext.insertFloat(this._domElement,this._domElement.getParent(),true); else if(this.isPositioned() == false) this.insertInFlowDOMElement(formattingContext,true); else {
			...
			if(this.isRelativePositioned() == true) this.insertInFlowDOMElement(formattingContext,false);
	...
	,insertInFlowDOMElement: function(formattingContext,render) {
		formattingContext.insert(this._domElement,this._domElement.getParent(),true,render);
	}
	,invalidate: function() {
	...
	,setBackgroundColor: function(value) {
		return this._backgroundManager.setBackgroundColor(value);
	}
	,getBackgroundColor: function() {
		return this._backgroundManager.getBackgroundColor();
	}
	,setBackgroundImage: function(value) {
		return this._backgroundManager.setBackgroundImage(value);
	}
	,getBackgroundImage: function() {
		return this._backgroundManager.getBackgroundImage();
	}
	,setBackgroundRepeat: function(value) {
		return this._backgroundManager.setBackgroundRepeat(value);
	}
	,getBackgroundRepeat: function() {
		return this._backgroundManager.getBackgroundRepeat();
	}
	,setBackgroundSize: function(value) {
		return this._backgroundManager.setBackgroundSize(value);
	}
	,getBackgroundSize: function() {
		return this._backgroundManager.getBackgroundSize();
	}
	,setBackgroundClip: function(value) {
		return this._backgroundManager.setBackgroundClip(value);
	}
	,getBackgroundClip: function() {
		return this._backgroundManager.getBackgroundClip();
	}
	,setBackgroundPosition: function(value) {
		return this._backgroundManager.setBackgroundPosition(value);
	}
	,getBackgroundPosition: function() {
		return this._backgroundManager.getBackgroundPosition();
	}
	,setBackgroundOrigin: function(value) {
		return this._backgroundManager.setBackgroundOrigin(value);
	}
	,getBackgroundOrigin: function() {
		return this._backgroundManager.getBackgroundOrigin();
	}
...
cocktailCore.style.abstract.AbstractContainerStyle.prototype = $extend(cocktailCore.style.js.Style.prototype,{
	...
	,flowChildren: function(containingDOMElementData,viewportData,lastPositionedDOMElementData,containingDOMElementFontMetricsData,formattingContext) {
		...
		if(formattingContext == null) {
			formattingContext = this.getformattingContext();
			childrenFormattingContext = this.getformattingContext();
		} else childrenFormattingContext = this.getformattingContext(formattingContext);
	...
	,insertInFlowDOMElement: function(formattingContext,render) {
		...
		if(this.establishesNewFormattingContext() == true) formattingContext.insert(this._domElement,this._domElement.getParent(),true,render1); else formattingContext.insert(this._domElement,this._domElement.getParent(),false,render1);
	}
		...
		if(isFirstPositionedAncestor == true) {
			...
			while(_g1 < _g) {
				...
				if(positionedDOMElementData.style.isRelativePositioned() == true) {
					var childrenBoxesData = positionedDOMElementData.formattingContext.getBoxesData(positionedDOMElementData.style.getDOMElement().getParent());
					var _g3 = 0, _g2 = childrenBoxesData.length;
					while(_g3 < _g2) {
						var i1 = _g3++;
						var _g5 = 0, _g4 = childrenBoxesData[i1].children.length;
						while(_g5 < _g4) {
							var j = _g5++;
							if(childrenBoxesData[i1].children[j].domElement == positionedDOMElementData.style.getDOMElement()) {
								var x = childrenBoxesData[i1].children[j].x;
								var y = childrenBoxesData[i1].children[j].y;
	...
	,insertTextElement: function(textElement,formattingContext,containingDOMElementData) {
		...
		while(_g1 < _g) {
			...
			case 0:
				var value = $e[2];
				formattingContext.insert(this.getTextFragmentDOMElement(textFragments[i],value),this._domElement,true,true);
				break;
			case 1:
	...
	,getformattingContext: function(previousformattingContext) {
		...
		if(this.establishesNewFormattingContext() == true) {
			var usedformattingContext;
			if(this.isPositioned() == true && this.isRelativePositioned() == false) usedformattingContext = null; else usedformattingContext = previousformattingContext;
			if(this.childrenInline() == true) formattingContext = new cocktailCore.style.formatter.InlineFormattingContext(containerDOMElement,usedformattingContext); else formattingContext = new cocktailCore.style.formatter.BlockFormattingContext(containerDOMElement,usedformattingContext);
...
var Blob = $hxClasses["Blob"] = function(inArena,inBits,inX,inY,inW,inH,inHotX,inHotY) {
	this.mArena = inArena;
	this.mBits = inBits;
	var inXFloat = inX;
	var inYFloat = inY;
	var inWFloat = inW;
	var inHFloat = inH;
	this.mRect = { x : inXFloat, y : inYFloat, width : inWFloat, height : inHFloat};
	this.mPoint = { x : 0.0, y : 0.0};
	this.mHotX = inHotX == null?inW / 2:inHotX;
	this.mHotY = inHotY == null?inH / 2:inHotY;
}
Blob.__name__ = ["Blob"];
Blob.prototype = {
	mArena: null
	...
	,mHotX: null
	,mHotY: null
	,draw: function(inX,inY) {
		this.mPoint.x = inX - this.mHotX;
		this.mPoint.y = inY - this.mHotY;
		this.mArena.drawImage(this.mBits.getNativeElement());
	}
	,__class__: Blob
}
var SkiState = $hxClasses["SkiState"] = { __ename__ : ["SkiState"], __constructs__ : ["FirstRun","Playing","Dead"] }
SkiState.FirstRun = ["FirstRun",0];
SkiState.FirstRun.toString = $estr;
SkiState.FirstRun.__enum__ = SkiState;
SkiState.Playing = ["Playing",1];
SkiState.Playing.toString = $estr;
SkiState.Playing.__enum__ = SkiState;
SkiState.Dead = ["Dead",2];
SkiState.Dead.toString = $estr;
SkiState.Dead.__enum__ = SkiState;
var Ski = $hxClasses["Ski"] = function() {
	this.tileImage = new cocktailCore.domElement.js.ImageDOMElement();
	this.tileImage.onLoad = this.init.$bind(this);
	this.tileImage.onError = this.initError.$bind(this);
	this.tileImage.load("tiles.png");
}
Ski.__name__ = ["Ski"];
Ski.rootDOMElement = null;
Ski.main = function() {
	Ski.rootDOMElement = new cocktailCore.domElement.js.BodyDOMElement();
	new Ski();
}
Ski.prototype = {
	mArena: null
	...
	,init: function(inBitmap) {
		this.mKeyDown = [];
		this.mArena = new cocktailCore.domElement.js.GraphicDOMElement();
		this.mArena.setWidth(640);
		this.mArena.setHeight(480);
		Ski.rootDOMElement.addChild(this.mArena);
		this.mTree = new Blob(this.mArena,inBitmap,4,0,54,64,null,62);
		this.mPlayerLeft = new Blob(this.mArena,inBitmap,110,16,26,40);
		this.mPlayerDown = new Blob(this.mArena,inBitmap,70,12,26,46);
		this.mPlayerRight = new Blob(this.mArena,inBitmap,148,16,26,40);
		var timer = new haxe.Timer(25);
		timer.run = this.OnEnter.$bind(this);
		this.mArena.setOnKeyDown(this.OnKeyDown.$bind(this));
		this.mArena.setOnKeyUp(this.OnKeyUp.$bind(this));
		this.mTrees = [];
		this.mTrees[Ski.mFieldHeight - 1] = null;
		var _g = 0;
		while(_g < 500) {
			var i = _g++;
			var x = Math.random() * 700 - 30;
			var y = Std["int"](Math.random() * (Ski.mFieldHeight - 250)) + 250;
			if(this.mTrees[y] == null) this.mTrees[y] = [];
			this.mTrees[y].push(x);
		}
		var mScoreTextContainer = new cocktailCore.domElement.js.ContainerDOMElement();
		this.mScoreText = new cocktailCore.textElement.js.TextElement("");
		mScoreTextContainer.setX(10);
		mScoreTextContainer.setY(10);
		mScoreTextContainer.setWidth(300);
		Ski.rootDOMElement.addChild(mScoreTextContainer);
		var mTopScoreTextContainer = new cocktailCore.domElement.js.ContainerDOMElement();
		this.mTopScoreText = new cocktailCore.textElement.js.TextElement("");
		mTopScoreTextContainer.addText(this.mTopScoreText);
		mTopScoreTextContainer.setX(300);
		mTopScoreTextContainer.setY(10);
		mTopScoreTextContainer.setWidth(300);
		Ski.rootDOMElement.addChild(mTopScoreTextContainer);
		this.mTopScore = 0;
		this.CheckTopScore(1000);
		this.mLastStep = haxe.Timer.stamp();
		this.Reset();
		this.mState = SkiState.FirstRun;
	}
	,CheckTopScore: function(inScore) {
		if(inScore > this.mTopScore) {
			this.mTopScore = inScore;
			var s = Std["int"](this.mTopScore * 0.1);
			this.mTopScoreText.setText("TopScore:" + s + "0");
		}
	}
	,Reset: function() {
		this.mPlayerX = 320;
		this.mPlayerY = 20;
		this.mScore = 0;
		this.mStepsPerSecond = 100;
	}
	,Update: function() {
		if(this.mState == SkiState.Playing) {
			var dx = this.mKeyDown[37]?-1:this.mKeyDown[39]?1:0;
			this.mPlayerX += dx * 0.3;
			if(this.mPlayerX < 12) this.mPlayerX = 12;
			if(this.mPlayerX > 628) this.mPlayerX = 628;
			this.mPlayerY += 1;
			if(this.mPlayerY > Ski.mFieldHeight) this.mPlayerY -= Ski.mFieldHeight;
			this.mScore += 1.0;
			this.mStepsPerSecond = 100 + this.mScore * 0.01;
			var row = this.mTrees[Std["int"](this.mPlayerY)];
			if(row != null) {
				var _g = 0;
				while(_g < row.length) {
					var x = row[_g];
					++_g;
					if(Math.abs(x - this.mPlayerX) < 15) {
						this.CheckTopScore(this.mScore);
						this.mState = SkiState.Dead;
					}
				}
			}
		}
	}
	,Render: function(inExtra) {
		var scroll_y = this.mPlayerY - 60;
		this.mArena.clear();
		var fill = cocktail.domElement.FillStyleValue.monochrome({ color : 16777215, alpha : 100});
		var line = cocktail.domElement.LineStyleValue.none;
		this.mArena.beginFill(fill,line);
		this.mArena.drawRect(0,0,640,480);
		this.mArena.endFill();
		var blob = this.mKeyDown[37]?this.mPlayerLeft:this.mKeyDown[39]?this.mPlayerRight:this.mPlayerDown;
		blob.draw(this.mPlayerX,this.mPlayerY - scroll_y);
		var _g1 = -10, _g = 560;
		while(_g1 < _g) {
			var y = _g1++;
			var field_y = Std["int"](scroll_y + y) % Ski.mFieldHeight;
			var row = this.mTrees[field_y];
			if(row != null) {
				var _g2 = 0;
		...
		if(this.mState == SkiState.FirstRun) this.mScoreText.setText("Press any key to start"); else if(this.mState == SkiState.Playing) {
			var s = Std["int"](this.mScore * 0.1);
			this.mScoreText.setText("Score:" + s + "0");
		} else {
			var s = Std["int"](this.mScore * 0.1);
			if(this.mScore >= this.mTopScore) this.mScoreText.setText("Top Score! " + s + "0" + "    Press [space] to go again"); else this.mScoreText.setText("You scored " + s + "0" + "    Press [space] to try again");
		}
	}
	,OnKeyDown: function(key) {
		if(!this.mKeyDown[key.code]) {
			if(this.mState == SkiState.FirstRun) this.mState = SkiState.Playing; else if(this.mState == SkiState.Dead && key.code == 32) {
				this.Reset();
				this.mState = SkiState.Playing;
			}
			this.mKeyDown[key.code] = true;
		}
	}
	,OnKeyUp: function(key) {
		this.mKeyDown[key.code] = false;
	}
	,OnEnter: function() {
		var now = haxe.Timer.stamp();
		var steps = Math.floor((now - this.mLastStep) * this.mStepsPerSecond);
		this.mLastStep += steps / this.mStepsPerSecond;
		var fractional_step = (now - this.mLastStep) * this.mStepsPerSecond;
		var _g = 0;
		while(_g < steps) {
			var i = _g++;
			this.Update();
		}
		this.Render(fractional_step);
	}
	,__class__: Ski
}
...
cocktailCore.style.computer.BackgroundStylesComputer.getComputedBackgroundSize = function(backgroundSize,backgroundPositioningArea,intrinsicWidth,intrinsicHeight,intrinsicRatio,emReference,exReference) {
	...
	case 2:
		var value = $e[2];
		computedBackgroundSize = { width : cocktailCore.style.computer.BackgroundStylesComputer.getBackgroundSizeStyleDimensionData(value.x,value.y,Math.round(backgroundPositioningArea.width),intrinsicWidth,intrinsicHeight,intrinsicRatio,emReference,exReference), height : cocktailCore.style.computer.BackgroundStylesComputer.getBackgroundSizeStyleDimensionData(value.y,value.x,Math.round(backgroundPositioningArea.height),intrinsicHeight,intrinsicWidth,intrinsicRatio,emReference,exReference)};
		break;
	}
	return computedBackgroundSize;
}
cocktailCore.style.computer.BackgroundStylesComputer.getBackgroundSizeStyleDimensionData = function(value,opositeBackgroundSizeStyleDimensionValue,backgroundPositioningAreaDimension,intrinsicDimension,opositeIntrinsicDimension,intrinsicRatio,emReference,exReference) {
	...
	case 1:
		...
		break;
	case 2:
		if(intrinsicDimension != null && opositeBackgroundSizeStyleDimensionValue == cocktail.style.BackgroundSizeStyleDimensionValue.auto) backgroundSizeStyleDimension = intrinsicDimension; else if(opositeIntrinsicDimension != null && intrinsicRatio != null) backgroundSizeStyleDimension = Math.round(opositeIntrinsicDimension * intrinsicRatio); else backgroundSizeStyleDimension = Math.round(cocktailCore.unit.UnitManager.getPixelFromPercent(100,backgroundPositioningAreaDimension));
		break;
	}
...
cocktailCore.style.formatter.FormattingContext = $hxClasses["cocktailCore.style.formatter.FormattingContext"] = function(domElement,previousformattingContext) {
	...
	if(previousformattingContext != null) this._floatsManager.addFloats(previousformattingContext);
	this._formattingContextData = this.initFormattingContextData(this._containingDOMElement);
	this._formattingBoxesData = new Array();
	this._currentBoxesData = new Array();
}
...
cocktailCore.style.formatter.FormattingContext.prototype = {
	...
	,_formattingBoxesData: null
	,_currentBoxesData: null
	...
	,getBoxesData: function(parentDOMElement) {
		return this.doGetBoxesData(parentDOMElement,this._formattingBoxesData);
	}
	,getCurrentBoxesData: function(parentDOMElement) {
		return this.doGetBoxesData(parentDOMElement,this._currentBoxesData);
	}
	,doGetBoxesData: function(parentDOMElement,targetBoxesData) {
		var boxesData = new Array();
		var _g1 = 0, _g = targetBoxesData.length;
		while(_g1 < _g) {
			var i = _g1++;
			if(targetBoxesData[i].parentDOMElement == parentDOMElement) boxesData.push(targetBoxesData[i]);
		}
		if(boxesData.length == 0) {
			var boxData = { parentDOMElement : parentDOMElement, children : new Array(), bounds : { x : 0.0, y : 0.0, width : 0.0, height : 0.0}, textDecorations : new Array()};
			targetBoxesData.push(boxData);
			boxesData.push(boxData);
		}
		return boxesData;
	}
	,insert: function(domElement,parentDOMElement,position,render) {
		this.doInsert(domElement,parentDOMElement,position,render);
	}
	,insertSpace: function(domElement,parentDOMElement) {
		this.doInsert(domElement,parentDOMElement,true,true);
	}
	...
	,insertFloat: function(domElement,parentDOMElement,render) {
		...
		this.placeFloat(domElement,parentDOMElement,floatData,render);
	}
	...
	,doInsert: function(domElement,parentDOMElement,position,render) {
		this.place(domElement,parentDOMElement,position,render);
		this.removeFloats();
	}
	...
	,place: function(domElement,parentDOMElement,position,render) {
	}
	,placeFloat: function(domElement,parentDOMElement,floatData,render) {
	}
...
cocktailCore.style.formatter.BlockFormattingContext.prototype = $extend(cocktailCore.style.formatter.FormattingContext.prototype,{
	place: function(domElement,parentDOMElement,position,render) {
		cocktailCore.style.formatter.FormattingContext.prototype.place.call(this,domElement,parentDOMElement,position,render);
...
cocktailCore.style.formatter.InlineFormattingContext.prototype = $extend(cocktailCore.style.formatter.FormattingContext.prototype,{
	...
	,insert: function(domElement,parentDOMElement,position,render) {
		...
		this._domElementInLineBox.push({ domElement : domElement, domElementType : cocktail.style.InlineBoxValue.domElement, parentDOMElement : parentDOMElement, render : render, x : 0, y : 0, position : position});
		if(position == true) cocktailCore.style.formatter.FormattingContext.prototype.insert.call(this,domElement,parentDOMElement,position,render);
	}
	,insertSpace: function(domElement,parentDOMElement) {
		var remainingWidth = this.getRemainingLineWidth();
		if(this._firstLineLaidOut == false) remainingWidth -= this._containingDOMElement.getStyle().getComputedStyle().textIndent;
		...
		this._domElementInLineBox.push({ domElement : domElement, domElementType : cocktail.style.InlineBoxValue.domElement, parentDOMElement : parentDOMElement, render : true, x : 0, y : 0, position : true});
	...
	,startNewLine: function(domElementWidth,isLastLine) {
		...
		if(this._domElementInLineBox.length > 0) {
			...
			var _g1 = 0, _g = this._currentBoxesData.length;
			while(_g1 < _g) {
				var i = _g1++;
				this._formattingBoxesData.push(this._currentBoxesData[i]);
			}
			this._currentBoxesData = new Array();
			this._domElementInLineBox = new Array();
	...
	,placeFloat: function(domElement,parentDOMElement,floatData,render) {
		cocktailCore.style.formatter.FormattingContext.prototype.placeFloat.call(this,domElement,parentDOMElement,floatData,render);
...
cocktail.style.BackgroundSizeStyleDimensionValue = $hxClasses["cocktail.style.BackgroundSizeStyleDimensionValue"] = { __ename__ : ["cocktail","style","BackgroundSizeStyleDimensionValue"], __constructs__ : ["length","percent","auto"] }
...
cocktail.style.BackgroundSizeStyleDimensionValue.auto = ["auto",2];
cocktail.style.BackgroundSizeStyleDimensionValue.auto.toString = $estr;
cocktail.style.BackgroundSizeStyleDimensionValue.auto.__enum__ = cocktail.style.BackgroundSizeStyleDimensionValue;
...
cocktailCore.background.BackgroundManager.prototype = {
	...
	,__properties__: {set_backgroundClip:"setBackgroundClip",get_backgroundClip:"getBackgroundClip",set_backgroundPosition:"setBackgroundPosition",get_backgroundPosition:"getBackgroundPosition",set_backgroundSize:"setBackgroundSize",get_backgroundSize:"getBackgroundSize",set_backgroundOrigin:"setBackgroundOrigin",get_backgroundOrigin:"getBackgroundOrigin",set_backgroundRepeat:"setBackgroundRepeat",get_backgroundRepeat:"getBackgroundRepeat",set_backgroundImage:"setBackgroundImage",get_backgroundImage:"getBackgroundImage",set_backgroundColor:"setBackgroundColor",get_backgroundColor:"getBackgroundColor"}
}
